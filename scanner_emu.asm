...
;from 3x4 = 0000cH		 
		...
		portc2 equ 0Ch		;Input switch and output LED for scan complete
...
; intialize ds, es,ss to start of RAM
		...
		;initialize 8255B
		;I/O Mode(1), Port A Mode0(00), Port A o/p(0), Port C Upper o/p(0), Port B Mode0 (0), Port B o/p(0), Port C Lower i/p(1)
		mov al,10000001b
		out creg2,al
		...
		;poll for the start scan switch
readswitch:		
		in al,portc2
		and al,1h
		cmp al,1
...
;subroutine for software delay of 1ms
delay_1ms:
		;software delay for 1ms
		push cx
xd:		mov cx,109
		loop xd
		pop cx
...
adc_isr:
		...
		in al,porta1
		cmp al, 80h 			;if greater than 128(mid_point of 0 to 255), it is a white pixel else it is a black pixel
		jge white
		
		;black
		;dl is storing the current pixel out of the 8 pixels being scanned.
		;bx stores the current photodiode being used to get data.
		;si stores the address where data from PD1 is getting stored
		mov ah,8
		sub ah,dl
		push cx
		mov cl,ah		
		...
		;make OE low, PC3 is connected to OE of ADC0808
		and al,11110111b
		out portc1,al
white:				
iret
